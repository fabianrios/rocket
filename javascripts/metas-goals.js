	...
	$('.save-goal').click(function (event) {
		...
		$('.goal-16-small').each(function (index) {
			if($(this).attr("id") != 'prime-goal-item')
			{
				...
				$('<input type="hidden" class="user_goal_info" value="' + monthDiff(new Date(firstDate.getFullYear(), firstDate.getMonth(), 1), date) + '" name="user_goal_term_' + number + '" id="user_goal_term_' + number + '" />').appendTo($('#diagnostics_form'));
				number++;
			}
		});
