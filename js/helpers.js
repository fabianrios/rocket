...
function displayLoading(elementId)
{
	if((typeof document.getElementById(elementId) != 'undefined') && (elementId != ''))
	{
		...
		document.getElementById(elementId).innerHTML = '<div class="descubra-question" style="display:visible;"><!-- Loading --><p class="text-center"><img src="/private/images/loading.gif" alt="loading" width="300" height="215" /></p></div><!-- End Loading -->';
	}
}
...
function menuDisplay(idElement)
{
	var idsSon = 'son_' + idElement;
	with(document.getElementById(idElement))
	{
		if(className == 'abierto')
		{
			className = 'cerrado';
			if(document.getElementById(idsSon))
				document.getElementById(idsSon).style.display='none';
		}
		else
		{
			className = 'abierto';	
			if(document.getElementById(idsSon))
				document.getElementById(idsSon).style.display='';
		}
	}
}

function hideImagesStage()
{
	$$(".contenedor2").each(function (element) {
...
function templeAll(idElement)
{
	var menu		= 'mm_' + idElement;
	var listMenu	= 'dd_' + idElement;
...
function hideImagesStage(idElement)
{
	var idElementPrimary = idElement + '_stages';
	var idLinkElement	 = 'stage_'+idElement;
	var locked = false;  
	$$(".contest-description").each(function (element) {
					element.style.display = 'none';
	...
	if(!locked)
		{
			locked = true;
			currentElement = document.getElementById(idElementPrimary);
			/*oculta
			new Effect.Fade(currentElement, {
				duration: this.settings['duration']});*/
			//muestra
			//
			new Effect.Appear(currentElement, {
					duration: 0.6,
					afterFinish: function () {
						locked = false;
					}
				});
			document.getElementById(idLinkElement).className = 'selected';	
		}			
}
...
function alertSpecialChars(str) {
	str.replace('Á', '\xC1').replace('É', '\xC9').replace('Í', '\xCD').replace('Ó', '\xD3').replace('Ú', '\xDA').replace('Ñ', '\xD1;').replace('á', '\xE1').replace('é', '\xE9').replace('í', '\xED').replace('ó', '\xF3').replace('ú', '\xFA').replace('ñ', '\xF1').replace('¿', '\xBF').replace('¡', '\xA1');
	alert(str);
}
