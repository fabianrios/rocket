	 ...
	 google.load("visualization", "1", {packages:["corechart"]});
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
		  ...
		  <?php
          $firstMonth = UserEnvelopeLogHelper::retrieveUserEnvelopeLogs("AND user_id = " . $user->__Get('user_id') . " AND user_envelope_id = " . $userEnvelope->__get('user_envelope_id') . " ORDER BY user_envelope_log_date ");
		...
		<div class="row">
			<div class="twelve columns"><!-- Main Panel Width -->
				<div class="panel header-explorar"> <!-- panel -->
					...
					<div class="ribbon">
						<h2><span class="head-e"><span class="descubra-icon" aria-hidden="true" data-icon="t"></span> </span><strong><?php echo $selectedUserEnvelope->__get('user_envelope_name')?></strong></h2>
...
<p class="left bugdet-li-p margin-top-10"><strong><a href="<?php echo APPLICATION_URL?>sobres-040.html">Resumen</a></strong></p></li>
								...
								foreach($userEnvelopes as $userEnvelope)
								{
									...
									?>
									<li class="budget-li<?php echo $class?>"><span  aria-hidden="true" data-icon="&#xe08f;" class="<?php echo $classExtra?> left fix-icon"></span>
									...
									<div class="container-2">
										<div class="row">
											<div class="four columns"><!-- elem. Agregar Btn -->
												<a href="<?php echo APPLICATION_URL?>#" class="add grey"><span class="add-expense" aria-hidden="true" data-icon="&#x3b;">Agregar nuevo gasto</span></a>
											...
											foreach($userEnvelopeExtraItemArray as $userEnvelopeExtraItem)
											{
												...
												<tr>
													...
													<td><?php echo formatDate('%b %d', $userEnvelopeExtraItem->__get('envelope_item_payday'))?></td>
												  	<td><a href="<?php echo APPLICATION_URL?>envelopes.controller/delete_envelope_extra/<?php echo $userEnvelopeExtraItem->__get('envelope_item_id')?>.html" onclick="return confirm('Esta seguro que desea borrar este gasto?')" title=""><span class="nav-icon" aria-hidden="true" data-icon="&#x62;"></span></a></td>
												</tr>
												<?php
											}
											?>
								...
								<div class="ultimos-gasto-items"><!-- Ultimos gastos Table -->
									<div class="container-2">
										<table summary="Table summary" width="100%" class="table-gastos">
											...
											<?php
											$envelopeItemArray = EnvelopeItemHelper::retrieveEnvelopeItems("AND user_id = " . $user->__get('user_id') . " AND user_envelope_id = " . $userEnvelope->__get('user_envelope_id') . " AND envelope_item_value != 0 AND envelope_bag_id != 0 AND envelope_item_save = 1 ORDER BY envelope_item_last_log_date DESC LIMIT 0,10 ");
											foreach($envelopeItemArray as $envelopeItem)
											{
