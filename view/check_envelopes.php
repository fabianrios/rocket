...
if(!isset($_SESSION["updated_envelopes"]) && isset($_SESSION["user_active"]))
{
	...
	foreach($userEnvelopes as $userEnvelope)
	{
		...
		// BUSCO GASTOS DIARIOS DESDE EL �LTIMO LOG
		for ($i = $startTime; $i <= $endTime; $i = $i + 86400) 
		{
			...
			$userEnvelopeExtraItemArray	= EnvelopeItemHelper::retrieveEnvelopeItems("AND user_id = " . $user->__get('user_id') . " AND user_envelope_id = " . $userEnvelope->__get('user_envelope_id') . " AND envelope_item_payday = '" . date('Y-m-d', $i) . "' AND envelope_bag_id = 0 AND envelope_item_save != 1 AND envelope_item_status = 1");		
			...
			if(count($userEnvelopeItemArray) > 0)
			{
				...
				foreach($userEnvelopeItemArray as $userEnvelopeItem)
				{
					...
					// ENTRA SI LA �LTIMA FECHA DE LOG ES MENOR QUE LA FECHA FINAL
