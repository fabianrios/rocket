    ...
    //GAUGES
	 ...
	 google.load("visualization", "1", {packages:["corechart"]});
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ['Mes', 'Gasto', 'Ejecucion del presupuesto'],
          <?php
          $firstMonth = UserEnvelopeLogHelper::retrieveUserEnvelopeLogs("AND user_id = " . $user->__Get('user_id') . " ORDER BY user_envelope_log_date ");
		  if(count($firstMonth) > 0)
		  {
		  	$firstLog 		= $firstMonth[0];
			$firstLogDate 	= $firstLog->__get('user_envelope_log_date');
		  }
		  else
		  {
		  	$firstLogDate = date('Y-m-d');	
		  }
		  $i = strtotime($firstLogDate);
		  $count = 0;
 		  $first = true;
		  while($i <= strtotime(date('Y-m-d')))
		  {

			 $nextCut = date('Y-m-d', strtotime(date('Y-m-01', $i) . '+1 month'));
			 $envelopeLogs = UserEnvelopeLogHelper::retrieveUserEnvelopeLogs(" AND user_id = " . $user->__get('user_id') . " AND user_envelope_log_date >= '" . date('Y-m-01', $i) . "' AND user_envelope_log_date <= '" . $nextCut . "'");
			 
			 $monthTotal 		= 0;
			 $totalPeriodBudget = 0;
			 foreach($envelopeLogs as $envelopeLog)
			 {
			 	$data 				= unserialize($envelopeLog->__get('user_envelope_log_data'));
				$totalPeriodBudget	+= $data["UserEnvelope"]->__get('user_envelope_budget'); 
				if(isset($data["EnvelopeItems"]))
				{
					foreach($data["EnvelopeItems"] as $envelopeItem)
					{
						$monthTotal += $envelopeItem->__get('envelope_item_value');
					}
				}
			 }
		  	...
		  	echo "['" . formatDate("%B %Y", date('Y-m-d', $i)) . "', " . $monthTotal . ", " . (($monthTotal / $totalPeriodBudget) * 100) . "]";			 
			 $i = strtotime($nextCut);
			 $count++;
			 if($count >= 12)
		  		break;
        ...
        var options = {
          vAxes: [{title: "Monto"}, {title: "%"}],
          hAxis: {title: "Mes"},
          seriesType: "line",
          series: {0: {type: "bars"}, 1:{targetAxisIndex:1}, vAxes:{1:{title:'Porcentaje'}}}
        };
							...
							<div class="eleven columns centered">
								<div class="bugdet-innerheader clearfix">
									<div class="budget-ribbon">
										<div class="budget-title left"><h2 class="left white txt-shadow-black"><strong class="whitetxt"><span class="whitetxt" aria-hidden="true" data-icon="t"></span> Resumen</strong></h2> <span class="sprite-1 info left">icon</span></div><div class="sprite-1 ribbon-detail left">icon</div>
									</div>
					...
					<div class="budget-main clearfix"><!-- Budget Main -->
						<div class="budget-col1 left"><!-- Col 1 Sidebar -->
							<ul class="budget-ul clearfix">
								...
								foreach($userEnvelopes as $userEnvelope)
								{
									...
									$currentTotalExpenses 	+= UserEnvelopeHelper::getPeriodExpenses($userEnvelope);
									$currentTotalBudget		+= intval($userEnvelope->__get('user_envelope_budget'));
								}
								?>
								<li class="budget-li active-sidebar"><div class="sprite-1 sobres-icon-2 left margin-left">icon</div>
								<p class="left bugdet-li-p margin-top-10"><strong><a href="<?php echo APPLICATION_URL?>sobres-040.html" class="whitetxt">Resumen</a></strong><br /><span class="small grey">$<?php echo formatNumber($currentTotalExpenses);?> de $<?php echo formatNumber($currentTotalBudget)?> </small></p></li>
								<?php
						...
						<div class="budget-col2 left"><!-- Col 2 Mainbar -->
									...
									<div class="container-2">
										<div class="row">
											<div class="four columns"><!-- elem. Agregar Btn -->
												<a href="<?php echo APPLICATION_URL?>#" class="add grey"><span class="add-expense">Agregar nuevo gasto</span></a>
											...
											foreach($userEnvelopeExtraItemArray as $userEnvelopeExtraItem)
											{
												...
												<tr>
													...
													<td><?php echo formatDate('%b %d', $userEnvelopeExtraItem->__get('envelope_item_payday'))?></td>
												  	<td><a href="<?php echo APPLICATION_URL?>envelopes.controller/delete_envelope_item/<?php echo $userEnvelopeExtraItem->__get('envelope_item_id')?>.html" onclick="return confirm('Esta seguro que desea borrar este gasto?')" title="">_</a><span class="sprite-1 trash-icon">Icon</span></a></td>
												</tr>
												<?php
											}
											?>
											...
											foreach($envelopeItemArray as $envelopeItem)
											{
												...
												<tr>
													...
													<td><strong>$<?php echo formatNumber($envelopeItem->__get('envelope_item_value'))?></strong></td>
												  	<td><a href="<?php echo APPLICATION_URL?>envelopes.controller/delete_envelope_item/<?php echo $envelopeItem->__get('envelope_item_id')?>.html" onclick="return confirm('Esta seguro que desea borrar este gasto recurrente?')" title=""><span class="nav-icon" aria-hidden="true" data-icon="b"></span></a></td>
												</tr>
												<?php
											}
											?>
							...
							<div class="ultimo-gasto-block"><!-- Block: Ultimo gasto recurrente -->
								...
								<div class="ultimo-gasto-bar"><!-- barra azul -->
									<div class="container-2">
										<p class="no-margin"><strong>Ahorros recientes</strong></p>
									</div>
								</div><!-- barra azul -->
								...
								<div class="ultimos-gasto-items"><!-- Ultimos gastos Table -->
									<div class="container-2">
										<table summary="Table summary" width="100%" class="table-gastos">
											...
											<?php
											$envelopeItemArray = EnvelopeItemHelper::retrieveEnvelopeItems("AND user_id = " . $user->__get('user_id') . " AND envelope_item_value != 0 AND envelope_bag_id != 0 AND envelope_item_save = 1 ORDER BY envelope_item_last_log_date DESC LIMIT 0,10 ");
											foreach($envelopeItemArray as $envelopeItem)
											{
												$total = $envelopeItem->__get('envelope_item_value') * $envelopeItem->__get('envelope_item_save_since');
												?>
												<tr>
													<td><div class="sprite-1 recurrencia-icon">icon</div></td>
													<td><?php echo date('F d', strtotime($envelopeItem->__get('envelope_item_last_log_date')))?></td>
													...
													<td><?php echo UserEnvelopeHelper::retrievePeriod($envelopeItem->__get('envelope_item_recurrence'));?></td>
													<td><strong>$<?php echo formatNumber($envelopeItem->__get('envelope_item_value'))?></strong></td>
													<td><strong>$<?php echo formatNumber($total)?></strong></td>
