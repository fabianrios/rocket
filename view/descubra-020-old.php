		...
		<div class="row">
			<div class="nine columns"><!-- Main Panel Width -->
				<div class="panel header-explorar"> <!-- panel -->
					...
					<!-- Ribbon -->
					<div class="ribbon">
						<h2><span class="head-e"><span class="sprite-1 descubra-icon">Icon</span></span><strong>Identifica:</strong> <?php echo utf8_encode($group->__get('group_name'));?></h2>
					...
					<div class="shadow"><img src="<?php echo APPLICATION_URL?>images/shadow-1.png" alt="shadow-1" width="1024" height="20" /></div>
						<div class="text-center">
							...
							{
							?>
								<a href="javascript:window.history.back(1);" class="pretty-btn"><span class="whitetxt large baseline" aria-hidden="true" data-icon="x"> Anterior</a>
                            <?php
							}
							if ($tab == count($groups))
							{
							?>
								<a href="javascript:void(0);" onClick="document.getElementById('questions').action='<?php echo APPLICATION_URL?>descubra-030/<?php echo $productId?>.html'; document.getElementById('questions').submit();" class="pretty-btn save">Ver resultados</a>
							<?php
							}
							else
							{
							?>
                            	<a href="javascript:void(0);" onClick="document.getElementById('questions').action='<?php echo APPLICATION_URL?>descubra-020/<?php echo $productId?>/<?php echo $tab+1;?>.html'; "  class="pretty-btn save">Siguiente</a>
                            <?php
							}
							?>
						</div>
			...
			<div class="three columns"><!-- Sidebar -->
				<div class="bluebox-fixed">
				<p class="giant"><?php echo intval((($tab-1)/(count($groups)+1))*80);?><span class="percentage">%</span></p>
				<p class="text-center white"><strong class="whitetxt">Nivel de precisión</strong></p>
				<hr class="dark" />
