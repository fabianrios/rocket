...
class UserEnvelopeHelper
{
	...
	public static function getCurrentPeriodExpenses(&$userEnvelope)
	{
		...
		foreach($recurringData as $key => $value)
		{
			if((strpos($key, '_amount') !== false) && ($value != ''))
			{
				...
				/*while($firstEnvelopeDate < $currentDate)
				{*/
					switch($recurringData[$globalKey . '_periodicity']):
						...
						case 360:
							...
							break;
					endswitch;
					//if(($firstEnvelopeDate>=$envelopeInitialDate) && ($firstEnvelopeDate<=$currentDate))
					if(($recurringData[$globalKey . '_periodicity'] != 0) && ($recurringData[$globalKey . '_periodicity'] != ""))
		...
		$envelopeExtras			= EnvelopeExtraHelper::retrieveEnvelopeExtras("AND user_id = " . $userEnvelope->__get('user_id') . " AND envelope_id = " . $userEnvelope->__get('envelope_id') . " AND (envelope_extra_date >= '" . $userEnvelope->__get('user_envelope_date') . "' AND envelope_extra_date <= '" . date('Y-m-d') . "') ORDER BY envelope_extra_date DESC");
