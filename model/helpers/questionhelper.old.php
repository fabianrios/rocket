...
{
	public static function selectQuestions ( $extra = "", $extraTables = ""   )
	{
		$connection  = Connection::getInstance();
		$retrieveQuestionsSql    = "SELECT question_id
							         FROM question_questions" . $extraTables . "
								     WHERE question_state <> 'D'
								     " . $extra;
		return $connection->query($retrieveQuestionsSql);		
	}
	public static function retrieveQuestions ( $extra  = "", $extraTables = ""  )
	{
		$questions = array();
		
		$retrieveQuestionsResult = self::selectQuestions ( $extra, $extraTables  );
		
		while($questionRow = mysql_fetch_assoc($retrieveQuestionsResult["query"]))
			$questions[] = new Question($questionRow["question_id"]);
	...
	public static function retrieveNextQuestions ($productId, $orderId = null, $answer = null, $tablePrefix)
	{
		// ANSWER IS NULL
		if($answer == null)
		{
			...
			if($orderId == null)
			{
				$questionOrderArray = OrderHelper::retrieveOrders("AND product_id = " . $productId . " ORDER BY order_value", $tablePrefix);
				if(count($questionOrderArray) > 0)
				{
					$questionOrder 	=& $questionOrderArray[0];  
			...
			{
				$pastQuestionOrder = new Order($orderId);
				$questionOrderArray = OrderHelper::retrieveOrders("AND product_id = " . $productId . " AND order_value > " . $pastQuestionOrder->__get('order_value') . " AND parent_id != " . $pastQuestionOrder->__get('order_id') . " ORDER BY order_value", $tablePrefix);
				if(count($questionOrderArray) > 0)
				{
					$questionOrder 	=& $questionOrderArray[0];  
		...
		{
			$questionOrderArray = OrderHelper::retrieveOrders("AND product_id = " . $productId . " AND  parent_id = " . $orderId . " AND order_conditional = '" . $answer . "' ORDER BY order_value", $tablePrefix);
			if(count($questionOrderArray) > 0)
			{
				$questionOrder 	=& $questionOrderArray[0];  
			...
			{
				$questionOrderArray = OrderHelper::retrieveOrders("AND product_id = " . $productId . " AND  parent_id = " . $orderId . " AND order_conditional = 0 ORDER BY order_value", $tablePrefix);
				if(count($questionOrderArray) > 0)
				{
					$questionOrder 	=& $questionOrderArray[0];  
				...
				{
					$pastQuestionOrder = new Order($orderId);
					$questionOrderArray = OrderHelper::retrieveOrders("AND product_id = " . $productId . " AND  order_value > " . $pastQuestionOrder->__get('order_value') . " AND parent_id != " . $orderId . " ORDER BY order_value", $tablePrefix);
					if(count($questionOrderArray) > 0)
					{
						$questionOrder 	=& $questionOrderArray[0];  
